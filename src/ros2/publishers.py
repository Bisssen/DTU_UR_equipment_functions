...
class Ros2Publishers():
    ...
    def publish_ur_pose(self, pose: list[float]) -> None:
        if not self.validate_list_size(pose, 6):
            return
        self.ur_pose_publisher.publish(self.convert_to_ros2_pose(pose))
    ...
    def publish_ur_joints(self, joints_list: list[float]) -> None:
    ...
    def convert_to_ros2_jointstate(self, jointstate_list: list[float]) -> JointState:
        msg = JointState()
